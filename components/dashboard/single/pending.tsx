...
const SinglePending = ({ pending, user }) => {
  ...
  let authorAvatar = user.profile_picture;
  ...
  const [articleType, setArticleType] = useState(pending.type);
  const [title, setTitle] = useState(pending.title);
  const [desc, setDesc] = useState(pending.description);
  const [banner, setBanner] = useState(pending.banner);
  const [categs, setCategs] = useState(pending.categories);
  const [rt, setRT] = useState(pending?.readingTime);
  const [content, setContent] = useState(pending.content);
  ...
  useEffect(() => {
    ...
    categoriesEl.forEach((option) => {
      ...
      if (pending.categories.includes(option.innerText)) {
        selected = true;
        option.setAttribute(
          'style',
          'background-color: var(--bg-color-secondary); color: #fff; transition: background 0.1s ease-in-out',
  ...
  const handleUpdate = async () => {
    try {
      await axios.put(
        `${process.env.PENDING_URL}/${pending._id}`,
        {
          type: articleType,
          author: author,
          title: title,
  ...
  const handleDelete = async () => {
    try {
      await axios.delete(`${process.env.PENDING_URL}/${pending._id}`, {
        data: { author: author },
        withCredentials: true,
      });
      router.push('/dashboard/pending');
  ...
  const sanitizeData = () => ({
    __html: DOMPurify.sanitize(pending.content),
  });
  ...
  const shareOnTwitter = () => {
    const navUrl = `https://twitter.com/intent/tweet?url=${encodeURIComponent(
      url,
    )}&text=${encodeURIComponent(pending.title)}`;
    window.open(navUrl, '_blank');
  };
            ...
            {isEdit && (
              <div className={styles.formContainer} id="form-container">
                <div className={styles.form} id="update">
                  <h1 className={styles.form__header}>Update pending</h1>
                  ...
                  <div>
                    <select
                      ...
                      required>
                      <option value={pending.readingTime}>
                        {pending.readingTime}
                      </option>
                      {readingDuration.map((duration, index) => {
                        while (duration.value !== pending.readingTime) {
                          return (
                ...
                {isOpen && (
                  <div className={styles.modal} ref={ref}>
                    ...
                    <div className={styles.prompt}>
                      Are you sure you wanted to delete this pending article?
                    </div>
              ...
              <section className={styles.main}>
                <div className={styles.author}>
                  <div className={styles.author__info}>
                    <Link href={`/?author=${pending.author}`} legacyBehavior>
                      <a>
                        <img
                          src={pending.avatar}
                    ...
                    <span>{pending.author}</span>
                  </div>
                  {pending.lastUpdated && (
                    <time>
                      Last edited on
                      {' ' +
                        new Date(pending.updatedAt).toDateString().slice(4)}
                    </time>
                  )}
                </div>
                  ...
                  {shuffle(Array.from(nextPosts))
                    ...
                    .map(
                      (next: { title: string; _id: string }, index: number) => {
                        const checkTitle = next.title !== pending.title;
                        return (
