...
const SinglePost = ({ post, user }) => {
  ...
  const [articleType, setArticleType] = useState(post.type);
  ...
  const handleUpdate = async () => {
    try {
      await axios.put(
        `${process.env.POSTS_URL}/${post._id}`,
        {
          type: articleType,
          author: author,
          title: title,
  ...
  const handleDelete = async () => {
    try {
      await axios.delete(`${process.env.POSTS_URL}/${post._id}`, {
        data: { author: author },
        withCredentials: true,
      });
      router.push('/dashboard/posts');
