...
const Settings = ({ user }) => {
  ...
  useEffect(() => {
    ...
    const inputBoxes = Array.from(
      container.querySelectorAll('[data-target="#input-box"]'),
    );
  ...
  const updateEmail = async (e: { preventDefault: () => void }) => {
    ...
    try {
      ...
      if (
        ...
        setInfo2(err.response.data);
      } else setInfo2(err.response.data.message);
    }
  };
  ...
  const handleReset = async (e: { preventDefault: () => void }) => {
    ...
    try {
      ...
      if (
        ...
        setInfo3(err.response.data);
      } else setInfo3(err.response.data.message);
    }
  };
