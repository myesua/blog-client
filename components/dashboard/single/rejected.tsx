...
const SingleRejected = ({ rejected, user }) => {
  ...
  const [articleType, setArticleType] = useState(rejected.type);
  const [title, setTitle] = useState(rejected.title);
  const [desc, setDesc] = useState(rejected.description);
  const [banner, setBanner] = useState(rejected.banner);
  const [categs, setCategs] = useState(rejected.categories);
  const [rt, setRT] = useState(rejected?.readingTime);
  const [content, setContent] = useState(rejected.content);
  ...
  useEffect(() => {
    ...
    categoriesEl.forEach((option) => {
      ...
      if (rejected.categories.includes(option.innerText)) {
        selected = true;
        option.setAttribute(
          'style',
          'background-color: var(--bg-color-secondary); color: #fff; transition: background 0.1s ease-in-out',
  ...
  const handleUpdate = async () => {
    try {
      await axios.put(
        `${process.env.REJECTED_URL}/${rejected._id}`,
        {
          type: articleType,
          author: author,
          title: title,
  ...
  /**Handle rejected delete */
  const handleDelete = async () => {
    try {
      await axios.delete(`${process.env.REJECTED_URL}/${rejected._id}`, {
        data: { author: author },
        withCredentials: true,
      });
      router.push('/dashboard/rejected');
  ...
  const sanitizeData = () => ({
    __html: DOMPurify.sanitize(rejected.content),
  });
  ...
  const shareOnTwitter = () => {
    const navUrl = `https://twitter.com/intent/tweet?url=${encodeURIComponent(
      url,
    )}&text=${encodeURIComponent(rejected.title)}`;
    window.open(navUrl, '_blank');
  };
            ...
            {isEdit && (
              <div className={styles.formContainer} id="form-container">
                <div className={styles.form} id="update">
                  <h1 className={styles.form__header}>Update rejected</h1>
                  ...
                  <div>
                    <select
                      ...
                      required>
                      <option value={rejected.readingTime}>
                        {rejected.readingTime}
                      </option>
                      {readingDuration.map((duration, index) => {
                        while (duration.value !== rejected.readingTime) {
                          return (
                ...
                {isOpen && (
                  <div className={styles.modal} ref={ref}>
                    ...
                    <div className={styles.prompt}>
                      Are you sure you wanted to delete this rejected article?
                    </div>
              ...
              <section className={styles.main}>
                <div className={styles.author}>
                  <div className={styles.author__info}>
                    <Link href={`/?author=${rejected.author}`} legacyBehavior>
                      <a>
                        <img
                          src={rejected.avatar}
                    ...
                    <span>{rejected.author}</span>
                  </div>
                  {rejected.lastUpdated && (
                    <time>
                      Last edited on
                      {' ' +
                        new Date(rejected.lastUpdated).toDateString().slice(4)}
                    </time>
                  )}
                </div>
                      ...
                      <div className={styles.related__child}>
                        {tips.slice(0, 3).map((tip: any, index: number) => {
                          const match = tip.categories.some((i: any) =>
                            rejected.categories.includes(i),
                          );
                          if (match == true)
                  ...
                  {shuffle(Array.from(nextPosts))
                    ...
                    .map(
                      (next: { title: string; _id: string }, index: number) => {
                        const checkTitle = next.title !== rejected.title;
                        return (
